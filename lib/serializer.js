(function () {
  ...
  function getParentAndSiblings(element) {
    if (!element) return { parent: null, siblings: [] };
    const parent = element.parentElement;
    if (parent) {
      return { parent, siblings: Array.from(parent.children) };
    }
    const parentNode = element.parentNode;
    if (parentNode && parentNode.nodeType === Node.DOCUMENT_FRAGMENT_NODE && parentNode.host) {
      return { parent: parentNode.host, siblings: Array.from(parentNode.children) };
    }
  ...
  function serializeElement(element, options, depth = 0) {
    ...
    // Serialize shadow root children first (Declarative Shadow DOM)
    if (element.shadowRoot) {
      ...
      for (const child of element.shadowRoot.childNodes) {
        if (child.nodeType === Node.TEXT_NODE) {
          ...
          if (text) {
            shadowParts.push(escapeHtml(text));
          }
          continue;
        }
        ...
        if (child.nodeType === Node.ELEMENT_NODE) {
          ...
          if (serialized) {
            shadowParts.push(serialized);
          }
        }
      }
      childParts.push(`<template shadowrootmode="open">${shadowParts.join('')}</template>`);
    }
  ...
  function buildStructure(element, options, depth = 0) {
    ...
    if (element.shadowRoot) {
      ...
      lines.push(`${shadowIndent}#shadow-root`);
      for (const child of element.shadowRoot.children) {
        lines.push(...buildStructure(child, options, depth + 2));
      }
    }
  ...
  function buildAccessibilitySummary(element, options, depth = 0) {
    ...
    if (element.shadowRoot) {
      ...
      lines.push(`${shadowIndent}#shadow-root`);
      for (const child of element.shadowRoot.children) {
        lines.push(...buildAccessibilitySummary(child, options, depth + 2));
      }
    }
  ...
  function generateCssPath(element) {
    ...
    while (current && current.nodeType === Node.ELEMENT_NODE) {
      ...
      const { parent, siblings } = getParentAndSiblings(current);
      if (parent) {
